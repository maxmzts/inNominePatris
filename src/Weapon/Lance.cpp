...
:   Weapon(engine), 
    ...
    portal(), 
    pinchSprite("./src/resources/Pinch_Animation_Lance.png"),
    pinchAnimation(pinchSprite) 
{
    spriteFacade.loadTexture("./src/resources/Weapons/lance.png"); // Cargar textura usando el Façade
...
void Lance::attack(sf::Vector2f position, sf::Vector2f direction) {
    if (attackTimer <= 0.f) {
        ...
        SFXManager::getInstance().addEffect("./src/resources/sfx/spear.wav", 60.f, pitch);
...
void Portal::loadAnimationFrames() {
    for (int i = 1; i <= 7; i++) {
        sf::Texture texture;
        std::string filePath = "./src/resources/portal-animation/portal1_frame_" + std::to_string(i) + ".png";
