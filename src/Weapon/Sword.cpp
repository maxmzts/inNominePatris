...
:   Weapon(engine),
    ...
    slashSpriteFacade("./src/resources/Horizontal_Slash_Sword.png"),
    slashAnimation(slashSpriteFacade),
    canDoubleDash(false),
    consecutiveDashes(1) // Inicializa con 1 dash disponible por defecto
{    
    spriteFacade.loadTexture("./src/resources/Weapons/sword.png"); // Cargar textura usando el Façade
...
void Sword::attack(sf::Vector2f position, sf::Vector2f direction) {
    if (attackTimer <= 0.f) {
        ...
        SFXManager::getInstance().addEffect("./src/resources/sfx/stab.wav", 60.f, pitch);
...
bool Sword::useAbility() {
    ...
    if (consecutiveDashes > 0) {
        ...
        SFXManager::getInstance().addEffect("./src/resources/sfx/dash.wav", 50.f, pitch);
