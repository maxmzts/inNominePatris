...
void EnemyManager::updateProjectiles(float deltaTime, Character* player) {
    for (auto it = activeProjectiles.begin(); it != activeProjectiles.end();) {
        ...
        if (projectile->checkCollision(player)) {
            ...
            SFXManager::getInstance().addEffect("./src/resources/sfx/Projectile_sound.wav", 100.f, pitch);
            VFXManager::getInstance().addEffect("./src/resources/vfx/explosion64x64.png",player->getPosition(),{64 , 64},10,8.f);
